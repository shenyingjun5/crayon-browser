...
struct GatewayEvent {
  ...
  std::optional<input_proof::PlayerReference> player_reference;
...
enum class GatewayResult {
  kAccepted = 0,
  kDroppedInvalidPlayerReference,
...
struct GatewayStats {
  ...
  std::uint64_t dropped_invalid_player_reference_total = 0;
  std::uint64_t dropped_invalid_geometry_total = 0;
};
...
public:
  ...
  GatewayResult
  SubmitPlayerRemoved(std::uint32_t tab_id, std::uint64_t navigation_id,
                      const renderer::MediaObservation &observation,
                      input_proof::PlayerReference player_reference);
...
private:
  ...
  std::uint64_t dropped_invalid_player_reference_total_ = 0;
  std::uint64_t dropped_invalid_geometry_total_ = 0;
};
