...
bool SpawnChild(const std::wstring &executable, std::uint64_t nonce,
  ...
  const std::wstring purpose = L"content-health-" +
  ...
  HANDLE inherited_handles[] = {child_input, child_output, null_error};
...
void StopChild(Child *child, bool graceful) {
  ...
  if (graceful && child->input) {
    ...
    if (auto payload =
            content_host_ipc::Encode(content_host_ipc::Shutdown{}, &error)) {
...
class ContentHostProcess::Impl final {
 public:
  ...
  bool Enqueue(Message message) {
    CodecError error = CodecError::kInvalidValue;
    auto payload = content_host_ipc::Encode(message, &error);
 ...
 private:
  ...
  bool Decode() {
    for (;;) {
      ...
      CodecError error = CodecError::kInvalidValue;
      auto message = content_host_ipc::Decode(payload, &error);
      if (!message ||
  ...
  void Exited(core_client::CoreClientSupervisor *supervisor) {
    ...
    decoder_.Reset();
    Clear();
    if (supervisor->OnEvent(CoreClientEvent::kProcessExited, NowMilliseconds()))
...
bool ContentHostProcess::Start(std::string path) {
...
std::vector<Message> ContentHostProcess::Drain(std::size_t maximum) {
