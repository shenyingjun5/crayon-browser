...
std::string SerializeRecord(char kind, const std::string &key,
...
std::optional<PreferenceStore>
DeserializePreferences(const std::string &document,
...
std::optional<PreferenceStore>
LoadPreferencesFromFile(const std::string &path, PreferenceCodecError *error) {
  ...
  if (size < 0 || static_cast<std::uintmax_t>(size) > kMaxPreferenceFileBytes) {
    SetError(error, PreferenceCodecError::kLengthOverflow);
    return std::nullopt;
  }
  in.seekg(0);
