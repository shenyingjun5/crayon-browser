...
- 内容视图挂载使用独立的非零 `MountEpoch`；创建、关闭和结果分离，所有结果必须回带完整挂载身份。Shell 只接受当前 epoch，后端名称和 native view 句柄不进入本契约。
...
- 内容能力是闭合 bit set，只描述当前实例已验证可用的导航、快照、可信输入、媒体观察、缩放和查找能力；未知 bit 拒绝。它不是运行时后端清单，页面不能选择后端或声明用途/能力。
