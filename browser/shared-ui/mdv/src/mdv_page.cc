...
std::string RenderMdvDocument(const MdvPageSnapshot& snapshot,
  ...
  document << "<!doctype html><html lang=\"" << EscapeHtml(strings.language)
           ...
           << "<button type=\"button\" class=\"icon-button\" "
              "data-outline-toggle aria-pressed=\"false\" aria-label=\""
           << EscapeHtml(strings.outline_label) << "\" data-tooltip-title=\""
           << EscapeHtml(strings.outline_label) << "\" data-tooltip-hint=\""
           << EscapeHtml(strings.outline_label) << "\">"
           << icons::kBulletList << "</button>"
           << "<input id=\"md-search\" class=\"md-search-input\" "
              "type=\"text\" placeholder=\""
           << EscapeHtml(strings.search_placeholder) << "\" aria-label=\""
           << EscapeHtml(strings.search_placeholder) << "\">"
           << "<button type=\"button\" class=\"icon-button\" "
              "data-search-prev aria-label=\""
           << EscapeHtml(strings.search_prev) << "\">&uarr;</button>"
           << "<button type=\"button\" class=\"icon-button\" "
              "data-search-next aria-label=\""
           << EscapeHtml(strings.search_next) << "\">&darr;</button>"
           << "<span id=\"md-search-count\" role=\"status\"></span>"
           << "</nav><nav id=\"md-outline\" class=\"md-outline\" "
              "role=\"navigation\" aria-label=\""
           << EscapeHtml(strings.outline_label) << "\" hidden>"
              "<ol id=\"md-outline-list\"></ol></nav>"
              "<div id=\"md-tooltip\" role=\"tooltip\" hidden>"
  ...
  document << "<div class=\"md-panes\"><section class=\"md-source-pane\" "
           ...
           << EscapeHtml(strings.mermaid_fullscreen)
           << "\" hidden><div class=\"md-mermaid-view-body\"></div>"
              "<div class=\"md-mermaid-view-bar\">"
              "<button type=\"button\" class=\"mermaid-view-source\" "
              "aria-pressed=\"false\" data-mermaid-view=\"source\">"
           << EscapeHtml(strings.mermaid_source)
           << "</button>"
              "<button type=\"button\" class=\"mermaid-view-close\" "
              "data-mermaid-view=\"close\">"
           << EscapeHtml(strings.mermaid_close)
           << "</button></div></div>"
...
std::string RenderMdvStylesheet() {
  ...
  css << ":root{color-scheme:light dark;--mdv-border:#dadce0;"
         "--mdv-hover:#f1f3f4;--mdv-active:#e8f0fe;"
         "--mdv-active-fg:#1967d2;--mdv-panel:#fff;--mdv-tip:#3c4043;"
         "--mdv-tip-fg:#fff;--mdv-text:#202124;--mdv-focus:#1a73e8;}"
      << "*{box-sizing:border-box}"
      << "body{margin:0;font:14px/1.6 system-ui,-apple-system,sans-serif;"
         "color:var(--mdv-text);background:var(--mdv-panel);height:100vh;"
         "display:flex;flex-direction:column;overflow:hidden;}"
      << ".view-bar{display:flex;align-items:center;gap:4px;padding:6px 10px;"
         "border-bottom:1px solid var(--mdv-border);min-height:49px;flex:none;}"
      << ".icon-button{display:inline-flex;align-items:center;justify-content:"
         ...
         "transparent;background:transparent;color:inherit;border-radius:50%;"
      ...
      << ".icon-button:focus-visible{outline:2px solid var(--mdv-focus);outline-offset:0;}"
      ...
      << ".md-panes{display:flex;flex:1;min-height:0;overflow:hidden;}"
      << ".md-source-pane,.md-preview-pane{flex:1;min-width:0;padding:0 14px;}"
      << ".md-outline{border-bottom:1px solid var(--mdv-border);max-height:38vh;"
         "overflow:auto;background:var(--mdv-hover);}"
      << ".md-outline[hidden]{display:none;}"
      << ".md-outline ol{list-style:none;margin:0;padding:6px 0;}"
      << ".md-outline button{display:block;width:100%;text-align:left;"
         "background:none;border:0;color:inherit;font:inherit;cursor:pointer;"
         "padding:4px 12px;}"
      << ".md-outline button:hover,.md-outline button:focus-visible"
         "{background:var(--mdv-border);}"
      << ".md-search-input{width:150px;max-width:22vw;padding:3px 8px;"
         "border:1px solid var(--mdv-border);background:transparent;"
         "color:inherit;font:inherit;}"
      << ".md-search-input:focus{outline:1px solid var(--mdv-active-fg);}"
      << "#md-search-count{font-size:12px;min-width:44px;text-align:center;"
         "color:var(--mdv-active-fg);}"
      << ".md-source-pane{display:flex;flex-direction:column;overflow:hidden;}"
      << ".md-preview-pane{overflow:auto;padding:24px clamp(16px,4vw,48px);}"
      << "#md-preview{max-width:900px;margin:0 auto;overflow-wrap:anywhere;}"
      << "#md-preview>:first-child{margin-top:0;}"
      << "#md-preview table{border-collapse:collapse;max-width:100%;}"
      << "#md-preview th,#md-preview td{padding:8px 14px;border:1px solid var(--mdv-border);text-align:left;}"
      << "#md-preview th{background:var(--mdv-hover);font-weight:500;}"
      << "#md-preview blockquote{margin:16px 0;padding:4px 20px;border-left:3px solid var(--mdv-border);color:var(--mdv-active-fg);}"
      << ".md-toolbar{display:flex;align-items:center;flex-wrap:nowrap;gap:2px;"
         "padding:4px 6px;background:var(--mdv-hover);border-radius:24px;"
         "margin:10px 0;overflow-x:auto;overflow-y:hidden;scrollbar-width:"
      ...
      << ".structure-item{width:100%;height:36px;justify-content:flex-start;"
         "gap:10px;padding:7px 10px;flex:none;text-align:left;border-radius:8px;}"
      ...
      << "body:not([data-view=split]) .md-divider{display:none;}"
      ...
      << "pre code[data-mdv-mermaid-rendered=true]{display:block;overflow-x:"
         ...
         "cursor:zoom-in;}"
      << "pre code[data-mdv-mermaid-rendered=true]:focus-visible{outline:2px "
         "solid #3370ff;outline-offset:2px;}"
      ...
      << ".mdv-mermaid-error-card{display:block;margin:0 0 8px;padding:2px "
         "8px;border-radius:4px;background:rgba(192,54,57,.1);color:"
         "#c03639;font-size:12px;}"
      << ".md-mermaid-view{position:fixed;inset:0;z-index:40;display:flex;"
         "flex-direction:column;background:var(--mdv-panel);}"
      << ".md-mermaid-view[hidden]{display:none;}"
      << ".md-mermaid-view-body{flex:1;min-height:0;overflow:auto;"
         "display:flex;align-items:center;justify-content:center;padding:"
         "16px;}"
      << ".md-mermaid-view-body svg{max-width:100%;height:auto;}"
      << ".mdv-mermaid-view-source{align-self:stretch;margin:0;padding:12px;"
         "font:13px/1.6 ui-monospace,monospace;white-space:pre-wrap;"
         "word-break:break-word;}"
      << ".md-mermaid-view-bar{display:flex;justify-content:flex-end;gap:8px;"
         "padding:10px 14px;border-top:1px solid var(--mdv-border);}"
      << ".md-mermaid-view-bar button{padding:4px 12px;}"
      ...
      << ".md-confirm{display:none;position:fixed;inset:auto 0 0 0;margin:auto;"
         "max-width:420px;padding:24px;border:1px solid var(--mdv-border);"
         "border-radius:20px;background:var(--mdv-panel);z-index:10;"
         "box-shadow:0 8px 32px rgba(0,0,0,.2);bottom:24px;}"
      << ".md-confirm[data-show=true]{display:block;}"
      << ".md-confirm button,.md-mermaid-view-bar button{margin-right:8px;padding:8px 18px;"
         "border:1px solid var(--mdv-border);border-radius:20px;background:var(--mdv-panel);"
         "color:var(--mdv-active-fg);cursor:pointer;}"
      << ".md-status{color:crimson;padding:4px 14px;margin:0;flex:none;}"
      << ".md-status:empty{display:none;}"
      << "@media(max-width:680px){.icon-button{width:32px;height:32px;"
         "flex-basis:32px;padding:5px}.view-bar{min-height:45px}"
         ".md-source-pane{padding:0 10px}.md-preview-pane{padding:20px 16px}"
         ".md-confirm{left:12px;right:12px;bottom:12px}}"
      << "@media(prefers-reduced-motion:reduce){.icon-button{transition:none}}"
      << "@media(prefers-color-scheme:dark){:root{--mdv-border:#5f6368;"
         "--mdv-hover:#303134;--mdv-active:#394457;--mdv-panel:#202124;"
         "--mdv-text:#e8eaed;--mdv-focus:#8ab4f8;"
         "--mdv-active-fg:#8ab4f8;--mdv-tip:#e8eaed;--mdv-tip-fg:#202124;}"
...
void AppendMdvCoreScript(std::ostringstream& js) {
  ...
  js << "'use strict';"
     ...
     << "if(typeof "
        ...
        "resetMermaid();preview.innerHTML=state.preview;observeHighlights("
        "preview);observeMath(preview);observeMermaid(preview);"
        "if(typeof mdvRebuildOutline==='function'){mdvRebuildOutline();}}"
     << "if(typeof "
        "state.dirty==='boolean'){body.setAttribute('data-dirty',state.dirty?'"
        "true':'false');if(state.dirty){var oldBanner=document.querySelector('.md-status');"
        "if(oldBanner){oldBanner.textContent='';}}}"
     ...
     << "if(typeof state.banner==='string'){var "
        ...
        "textContent=state.banner;b.classList.toggle('md-ok',state.saveOk===true);"
        "b.setAttribute('role',state.saveOk===true?'status':'alert');}"
     << "}"
     << "window.mdvPush=apply;"
     << "var outlineNav=document.getElementById('md-outline');"
     << "var outlineList=document.getElementById('md-outline-list');"
     << "var outlineToggle=document.querySelector('[data-outline-toggle]');"
     << "function mdvOutlineJump(ordinal){var headings=preview.querySelectorAll('h1,h2,h3,h4,h5,h6');var target=headings[ordinal];if(target){target.scrollIntoView({block:'start'});}}"
     << "function mdvRebuildOutline(){if(!outlineNav||!outlineList||!preview){return;}"
     << "var headings=preview.querySelectorAll('h1,h2,h3,h4,h5,h6');"
     << "outlineList.innerHTML='';var stack=[];"
     << "headings.forEach(function(h,ordinal){var level=+h.tagName.charAt(1);"
     << "while(stack.length&&stack[stack.length-1]>=level){stack.pop();}"
     << "var li=document.createElement('li');var btn=document.createElement('button');"
     << "btn.type='button';btn.textContent=h.textContent;"
     << "btn.setAttribute('aria-label',h.textContent+' (level '+level+')');"
     << "btn.style.paddingLeft=(stack.length*14+8)+'px';"
     << "btn.addEventListener('click',function(){mdvOutlineJump(ordinal);});"
     << "li.appendChild(btn);outlineList.appendChild(li);stack.push(level);});"
     << "outlineNav.hidden=outlineList.children.length===0;"
     << "if(outlineToggle){outlineToggle.setAttribute('aria-pressed',outlineNav.hidden?'false':'true');outlineToggle.hidden=outlineNav.hidden;}}"
     << "var searchInput=document.getElementById('md-search');"
     << "var searchPrev=document.querySelector('[data-search-prev]');"
     << "var searchNext=document.querySelector('[data-search-next]');"
     << "var searchCount=document.getElementById('md-search-count');"
     << "var searchState={matches:[],cursor:0,truncated:false};"
     << "function mdvUpdateCount(){if(!searchCount){return;}"
     << "var q=searchInput?searchInput.value:'';"
     << "searchCount.textContent=q?(searchState.matches.length?"
        "String(searchState.cursor+1)+'/'+searchState.matches.length"
        "+(searchState.truncated?'+':''):'0/0'):'';}"
     << "function mdvShowMatch(){if(!searchState.matches.length){return;}"
     << "var at=searchState.matches[searchState.cursor];"
     << "source.setSelectionRange(at,at+searchInput.value.length);"
     << "var lines=source.value.slice(0,at).split('\\n').length-1;"
     << "source.scrollTop=Math.max(0,lines*18-source.clientHeight/2);}"
     << "function mdvRunSearch(){if(!searchInput||!source){return;}"
     << "var q=searchInput.value.toLowerCase();"
     << "var hay=source.value.toLowerCase();"
     << "searchState.matches=[];searchState.cursor=0;searchState.truncated=false;"
     << "if(q){var pos=0;"
     << "while(searchState.matches.length<200){var found=hay.indexOf(q,pos);"
     << "if(found<0){break;}"
     << "searchState.matches.push(found);pos=found+q.length;}"
     << "if(searchState.matches.length>=200&&hay.indexOf(q,pos)>=0){searchState.truncated=true;}}"
     << "mdvUpdateCount();mdvShowMatch();}"
     << "function mdvSearchStep(delta){if(!searchState.matches.length){return;}"
     << "searchState.cursor=(searchState.cursor+delta+searchState.matches.length)"
        "%searchState.matches.length;"
     << "mdvUpdateCount();mdvShowMatch();}"
     << "if(searchInput){searchInput.addEventListener('input',mdvRunSearch);"
     << "searchInput.addEventListener('keydown',function(e){"
     << "if(e.key==='Enter'){e.preventDefault();mdvSearchStep(e.shiftKey?-1:1);}"
     << "if(e.key==='Escape'){searchInput.value='';mdvRunSearch();}});}"
     << "if(searchPrev){searchPrev.addEventListener('click',function(){mdvSearchStep(-1);});}"
     << "if(searchNext){searchNext.addEventListener('click',function(){mdvSearchStep(1);});}"
     << "if(outlineToggle&&outlineNav){outlineToggle.addEventListener('click',function(){"
     << "outlineNav.hidden=!outlineNav.hidden;"
     << "outlineToggle.setAttribute('aria-pressed',outlineNav.hidden?'false':'true');"
     << "if(!outlineNav.hidden){var first=outlineList.querySelector('button');if(first){first.focus();}}});}"
     << "if(outlineNav){outlineNav.addEventListener('keydown',function(e){"
     << "var buttons=outlineList.querySelectorAll('button');if(!buttons.length){return;}"
     << "var current=document.activeElement;var index=Array.prototype.indexOf.call(buttons,current);"
     << "if(e.key==='ArrowDown'){e.preventDefault();buttons[Math.min(buttons.length-1,index+1<0?0:index+1)].focus();}"
     << "else if(e.key==='ArrowUp'){e.preventDefault();buttons[Math.max(0,index-1)].focus();}"
...
void AppendMdvMermaidScript(std::ostringstream& js) {
  ...
  js << "var mermaidObserver=null;"
     ...
     << "var firstScreen=Math.min(nodes.length,8);"
     << "for(var i=0;i<firstScreen;i++){queueMermaid(nodes[i]);}return;}"
     << "if(!mermaidObserver){mermaidObserver=new IntersectionObserver("
        "function(entries){for(var i=0;i<entries.length;i++){"
     << "if(entries[i].isIntersecting){mermaidObserver.unobserve(entries[i]."
        "target);queueMermaid(entries[i].target);}}},{root:null,rootMargin:"
        "'160px 0px',threshold:0});}"
     ...
     << "var card=node.querySelector('.mdv-mermaid-error-card');"
     << "if(card){card.remove();}"
     << "if(node.getAttribute('data-mdv-mermaid-rendered')==='true'){"
     << "var svg=node.querySelector('svg');if(svg){svg.remove();}"
     << "node.removeAttribute('data-mdv-mermaid-rendered');}}}"
     ...
     << "var mermaidViewBody=mermaidView?mermaidView.querySelector('.md-"
        "mermaid-view-body'):null;"
     << "var mermaidViewOpener=null;"
     << "function closeMermaidView(){if(!mermaidView){return;}"
     << "mermaidView.hidden=true;if(mermaidViewBody){mermaidViewBody."
        "innerHTML='';}"
     << "if(mermaidViewOpener){mermaidViewOpener.focus();mermaidViewOpener="
        "null;}}"
     << "function openMermaidView(block){if(!mermaidView||!mermaidViewBody||"
        "!block){return;}"
     << "var svg=block.querySelector('svg');if(!svg){return;}"
     << "mermaidViewOpener=block;mermaidViewBody.innerHTML='';"
     << "mermaidViewBody.appendChild(svg.cloneNode(true));"
     << "var toggle=mermaidView.querySelector('.mermaid-view-source');"
     << "if(toggle){toggle.setAttribute('aria-pressed','false');}"
     << "mermaidView.hidden=false;"
     << "var close=mermaidView.querySelector('.mermaid-view-close');"
     << "if(close){close.focus();}}"
     << "if(mermaidView){"
     << "mermaidView.addEventListener('click',function(event){"
     << "var action=event.target.closest?event.target.closest('[data-mermaid-"
        "view]'):null;if(!action){return;}"
     << "var kind=action.getAttribute('data-mermaid-view');"
     << "if(kind==='close'){closeMermaidView();return;}"
     << "if(kind==='source'){var existing=mermaidViewBody.querySelector('."
        "mdv-mermaid-view-source');"
     << "if(existing){existing.remove();action.setAttribute('aria-pressed',"
        "'false');return;}"
     << "var hiddenSource=mermaidViewOpener?mermaidViewOpener.querySelector('"
        ".mdv-mermaid-source'):null;"
     << "if(hiddenSource){var sourceView=document.createElement('pre');"
     << "sourceView.className='mdv-mermaid-view-source';"
     << "sourceView.textContent=hiddenSource.textContent;"
     << "mermaidViewBody.appendChild(sourceView);"
     << "action.setAttribute('aria-pressed','true');}}});"
     << "if(preview){preview.addEventListener('click',function(event){"
     << "var block=event.target.closest?event.target.closest('code[data-mdv-"
        "mermaid-rendered=\\\"true\\\"]'):null;"
     << "if(block){openMermaidView(block);}});}"
     << "document.addEventListener('keydown',function(event){"
     << "if(event.key==='Escape'&&!mermaidView.hidden){event.preventDefault();"
        "closeMermaidView();}});}";
}
