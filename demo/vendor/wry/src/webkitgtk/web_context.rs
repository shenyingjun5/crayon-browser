    ...
    F: Fn(crate::WebViewId, Request<Vec<u8>>, RequestAsyncResponder) + 'static,
  {
    ...
    self.os.context.register_uri_scheme(name, move |request| {
      ...
      if let Some(uri) = request.uri() {
        ...
        // [get-video demo patch] webkit2gtk 2.38 没有
        // webkit_uri_scheme_request_get_http_body（2.40 新增），上面的
        // http_body() 恒为 None 导致 body 为空、IPC invoke 参数丢失。
        // 兜底：demo 前端把 IPC body 挪进了 x-tauri-body 请求头，这里读回。
        let body = if body.is_empty() {
          let mut found: Option<String> = None;
          if let Some(headers) = request.http_headers() {
            headers.foreach(|k, v| {
              if k.eq_ignore_ascii_case("x-tauri-body") {
                found = Some(v.to_string());
              }
            });
          }
          match found {
            Some(s) => get_video_demo_percent_decode(&s),
            None => body,
          }
        } else {
          body
...
/// [get-video demo patch] 极简 percent-decode，配合上面的 x-tauri-body 兜底逻辑。
fn get_video_demo_percent_decode(s: &str) -> Vec<u8> {
  let b = s.as_bytes();
