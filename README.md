...
- **流地址失效**（央视 4K 专区老片实测场景：CDN 清档，主列表 200 但分片变体 404）：`Extractor::inspect_hls` 拉主列表时下钻变体，HTTP 4xx/5xx 即标「流地址已失效」，秒级完成、不用探针。网络层错误（超时/DNS）不下结论。
- **DRM**：流级结构检测（播放列表 `KEYFORMAT` / MPD `ContentProtection`）+ 全 DRM 站点名单（netflix 等）前置标记。
- **WASM 私有加扰**（央视频直播、央视 dh5/enc 加密流）：这类流**码流结构与干净流完全一致**——2026-08-02 逐项对比实证：TS 容器、PAT/PMT、SPS/PPS（重复 GOP 字节一致）、slice 头 Exp-Golomb 全部合法，无法静态区分；唯一可靠判定是**真实解码后看画面**（加扰流实测纯黑 mean=0/std=0 或灰屏碎片，正常内容 std 一般 >25）。

WASM 判定走**解码探针**（`src/probe.rs` 判定 + relay `/probeplayer` 探测页 + app 隐藏 webview）：解析结果出来后**不阻塞列表**，对命中央视家族域名（`drm::is_cctv_family`，仅作触发器不作结论）的候选后台逐个实测，异常者经 `probe-restriction` 事件异步给链接打上「受限，不能播放」（置灰禁点、不出投屏）；实测可播的（如央视纪录片点播）保持正常。探针综合判定三分：画面全平坦→加扰、播放器确定性报错（非超时）→加载失败、超时无数据→**不下结论**（宁漏不误）。另外播放时还有一层**通用兜底**：任何站点播放 6s 后抽一帧，画面平坦（std<1.5）且进度在走即标注受限——覆盖未知加扰站点。
