...
- 平台剩余重点：`CNT-18a/18b DONE` 已冻结 codec 并完成 Rust host，下一步严格推进 `CNT-18c READY`、18d；`PLT-M05b1 READY` 后串行 b2..b6/M05c，随后 `PLT-W05` 对称装配；`MDV-25 READY` 清除生产 fixture 后，`MDV-20/24 VERIFIED` 与 `MRT-09` 关闭 P0 Runtime 发布缺口。M05a 既有证据只代表基础壳/new-tab，不代表 CNT/Cast 全产品装配。
...
1. `CNT-18c READY`：将已完成的 Rust host 作为真实 Core 子进程接入 macOS CEF Browser，完成 spawn/pipe/health/kill/reap 与 gateway drain；再执行 18d 真 CEF E2E，之后推进 `CNT-19..21`。
