...
- **M05b2b（版本化本机 media-host 协议）**：状态 `IN_PROGRESS`；依赖 M05b2a VERIFIED。按 b2b1/b2b2 严格串行，增加独立 `crayon-media-host` 本机子进程与双端严格 codec/transport，把完整 URL 只保留在 Browser↔host 私有内存通道，向 UI 只返回 opaque candidate id/redacted origin/闭合 decision；禁止复用/扩张 CAAP、content-host CHV1、远程监听、SDK/UI/Relay。
