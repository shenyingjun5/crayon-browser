...
| MRT | [markdown-runtime-roadmap.md](markdown-runtime-roadmap.md) | Markdown Runtime Extension Framework：闭合扩展 API、Highlight/KaTeX 与后续图表/演示门禁 | `MRT-01..05 DONE`，`MRT-06 VERIFIED`（macOS 完成，Windows 待远程补验），`MRT-07 DONE`，`MRT-08 READY`；`MRT-06..09` 为 P0，`10..19` 分波次推进或仅做 gap analysis |
...
| 7 | `MRT-08` | READY | 接入 KaTeX inline/block facts、受控 HTML/MathML policy、离线资源/字体路由与 generation/lazy 生命周期 |
