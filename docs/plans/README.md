...
| MRT | [markdown-runtime-roadmap.md](markdown-runtime-roadmap.md) | Markdown Runtime Extension Framework：闭合扩展 API、Highlight/KaTeX 与后续图表/演示门禁 | `MRT-01..05 DONE`，`MRT-06 VERIFIED`（macOS 已闭合；Windows 自动化通过、真实 CEF 高亮待修复复验），`MRT-07..08 DONE`；`MRT-06..09` 为 P0，`10..19` 分波次推进或仅做 gap analysis |
