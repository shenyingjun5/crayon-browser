...
| ACT | [semantic-action-roadmap.md](semantic-action-roadmap.md) | Page/Action/Form/Media/Risk Map、action_id、前置条件和效果验证 | `ACT-01 DONE`、`ACT-02/03/04 VERIFIED`（2026-08-30）；`ACT-05` 起 READY |
...
| 6 | `ACT-01` | DONE | 语义地图/ChangeSet/effect/错误 schema 已冻结并入 golden 兼容窗口（e45d086）；`ACT-02/03/04 VERIFIED`（detail profile、单次 action handle、多信号 discovery 契约），`ACT-05` 起按依赖领取 |
