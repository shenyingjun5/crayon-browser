...
- Alloy 自动化增量（2026-09-05）：当前 catalog 已增长到 197 keys，generator `--check` 为 3 locales/9 files，正负向 6/6 PASS；Windows Debug/Release `ALL_BUILD` 退出 0，连续自动集合各 124/124 PASS（339.43s/313.04s），可信物理点击项再各 1/1 PASS（6.61s/4.52s），累计各 125/125。当前 Release staging 为 35 files/372,923,339 bytes/12 locale pak，主 DLL SHA-256=`DB7EAAD658A8976D111B170982D8BD15E6B7705348E59E9CDE9BFD798F61545A`，artifact scan、RG-006/009、security、diff check PASS。`fast` 首轮出现一次 content-host health pipe error 233，单项随后 30/30 及完整复跑 PASS；保留为非稳定风险，不宣称已修。上述自动化与受控 200% probe 均不替代下一条真实系统矩阵。
- 待闭合：还需安装/切换 `en-US`、`zh-TW` 和一种不支持语言的用户 UI 语言，从干净 Profile 完整重启并运行 header/JS/html-lang 与全 surface 矩阵；Narrator、简繁中文/英文 IME、浅/深、窄/宽、原生 200% DPI、安装/升级/回滚及三语言人工审校均为 `NOT_RUN`。这些外部条件连续阻塞后，LOC-07W 保持 `BLOCKED`；不得以自动化或注入输入冒充关闭。
