...
- `24W2b3 TODO`：把 advanced tabs、bookmarks、history/recently-closed、download shelf 等其余日用 adapter 接入同一产品状态流与可见入口；已拆为 `24W2b3a VERIFIED` 产品 owner/持久化流、`24W2b3b1 VERIFIED` advanced tabs/书签可见 surface、`24W2b3b2 TODO` history/download 可见 surface 与 `24W2b3b3 TODO` 跨窗口标签移动。24W2 原范围因 Cast、安全、多窗口/Profile 和日用 surface 合计超过单提交审查预算而拆为 a/b1/b2/b3，不改变验收或一期范围。
