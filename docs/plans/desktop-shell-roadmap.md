...
- `24W2b3 TODO`：把 advanced tabs、bookmarks、history/recently-closed、download shelf 等其余日用 adapter 接入同一产品状态流与可见入口；再拆为 `24W2b3a VERIFIED` 产品 owner/持久化流与 `24W2b3b TODO` 可见交互 surface。24W2 原范围因 Cast、安全、多窗口/Profile 和日用 surface 合计超过单提交审查预算而拆为 a/b1/b2/b3，不改变验收或一期范围。
