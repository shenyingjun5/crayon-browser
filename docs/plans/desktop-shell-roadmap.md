...
- 状态：IN_PROGRESS；依赖 08W VERIFIED。单一目标：将既有 `browser/session` 恢复策略绑定 Windows Alloy window/tab snapshot 与恢复执行，覆盖正常退出和崩溃 checkpoint；不切默认入口。
...
- `24W2b2b READY`：把 Profile/request-context factory 与无痕隔离接入 production Host；保持一窗口一 Profile、regular context 复用、每个无痕窗口 memory-only 且零持久化。
- `24W2b2c TODO`：把 session checkpoint/restore 接入 production Host；只恢复 regular Profile 的有界窗口/tab/advanced metadata，损坏、跨 Profile 与部分失败显式拒绝。
