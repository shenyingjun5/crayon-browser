...
| HM-10 | TODO | HM-08,HM-09,AGT-14 | `apps/harmony/**`, `platform/harmony/**` | PC 窗口/键鼠/文件/外部交接，以及适用 CAAP R0/R1 产品装配 | PC-form UI/device；AG-006/012/014 | VH |
| HM-11 | TODO | HM-10,SDK-14 | `tests/**`, `apps/harmony/**` | 鸿蒙电脑 Direct/Relay/外部交接与适用 Agent 读取真实端到端验证 | `E2E-001..005`,`AG-006`; device | VH |
...
- 浏览器只负责 LAN Direct/Relay；无媒体路由时交接给独立客户端，不实现采集、编码或 WebRTC。CAAP 逻辑协议与桌面共用，本机 transport 和工具支持按鸿蒙电脑能力降级，不能私建另一套 Agent API。
