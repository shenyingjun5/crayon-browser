...
| MRT-09 | TODO | MDV-20,MDV-25,MRT-06,MRT-08 | `tests/e2e/desktop`,`tools/repo-guard`,`docs/current`,`docs/plans` | P0 Runtime 收口：CommonMark/GFM + Highlight + Mermaid Full + KaTeX 的双平台/包体/性能/安全总 Review | MR-001..005,MR-008/012；P0/P1=0 |
...
- 状态：`DONE`；依赖 `MRT-05 DONE`。单一目标是把已冻结的 highlight.js core/grammar 作为一个闭合 fence adapter 接入 `crayon://mdv`，仅对精确 allowlist 语言按可见 block 懒加载并应用 MDV 自有浅/深色 token 主题；普通、纯文本、未知或失败 fence 保留 md4c 已转义的 Level A code block。
...
- Windows blocker 修复：`crayon` standard/secure/display-isolated scheme 按 CEF standard custom scheme 语义增加 `CEF_SCHEME_OPTION_CORS_ENABLED`，不开放 `FETCH_ENABLED` 或 `CSP_BYPASSING`；lazy observer 以有界 `data-mdv-highlight-loading` 防重复，只有成功后 unobserve，失败会清除 loading 并保留观察以便重试，不增加轮询或高频日志；`hljs` 基础主题 class 只由 adapter 在 token DOM 成功提交后添加。共享 C++ closure 现在与 adapter/vendor 一致，有界跳过 Highlight.js `javascript <-> xml` 合法循环边，避免含 `javascript/js` 的文档整批回退；vendor check 对 UTF-8 CRLF worktree 先严格规范为 LF 再核对冻结字节长度/hash，Git blob/LF 契约未降级。
...
- Windows 构建/自动化收口（2026-08-28）：基于包含 `a7e5603` 的 `main` 与本机离线 CEF 150 Windows x64 分发，`cmake --preset windows-cef-debug` 通过；`cmake --build .cache\build\windows-cef-debug --config Debug --parallel 1 -- /nr:false` 与同命令 `--config Release` 均通过；`ctest --test-dir .cache\build\windows-cef-debug -C Debug --output-on-failure` 为 68/68（72.23 s），Release 为 68/68（66.32 s）。`node --test browser\shared-ui\markdown-runtime\tests\highlight_adapter.test.mjs` 5/5，`node --test tools\highlight\vendor.test.mjs` 5/5，`node tools\highlight\vendor.mjs --check` 为 `25 grammars, 124585 bytes`；新增真实多围栏回归先稳定复现 `decorated_blocks == 0`，确认并修复合法循环依赖后通过。首次 preset 在 `core.autocrlf=true` checkout 被 MRT-08 现有检查原样阻断为 `KaTeX asset does not match manifest: assets/katex.mjs`；为保持本任务不修改 KaTeX，仅临时物化 HEAD 的精确 LF blob 完成构建，随后恢复 Windows checkout，最终 diff 不含 `third_party/katex/**`。
- Windows 真实 CEF 收口（2026-08-28）：Release `CrayonBrowser.exe` 离线打开本地 fixture，亮色下首屏 canonical C++ 正确着色；`plaintext`/未知语言保持纯文本，恶意 `<img onerror>`/`<script>` 仅显示为字面文本。初始 Network 只有 `app.html/app.css/app.js/adapter/core/cpp` 六个同源请求；滚动到折叠下方后才新增 `css/graphql/xml/javascript/csharp`，随后 `js` 与 `c#` alias 均正确着色，11 个请求全部 200。Elements 显示三个成功节点均有 `hljs`、`data-mdv-highlighted="true"` 和 `data-mdv-node` generation fence；DevTools `prefers-color-scheme: dark` 真实渲染下深色 token 主题生效。
- Windows 资源/离线证据（2026-08-28）：DevTools Console 为空并显示 `No Issues`，Network 捕获中没有 `http/https`、CSP 或失败资源；进程 TCP 只出现 loopback `127.0.0.1`（本机代理/DevTools），没有非 loopback remote。custom scheme dynamic module import、viewport lazy 与完整冻结资源闭包均由上述真实请求序列直接验证；故障路径由 `mdv_page` contract 锁定 loading 清理、成功后 unobserve 和失败保留观察，不以静默吞错冒充通过。
- Windows 独立 Code Review v0.8（2026-08-28）：APPROVE，P0/P1/P2/P3=0。旧 Review 的 CORS/dynamic import、失败后不可重试、基础主题 class 与 CRLF vendor check finding 已关闭；复审确认无 `FETCH_ENABLED`/CSP bypass、新网络/文件权限、无界 timer/cache、热路径日志或 KaTeX/Mermaid 夹带。合法循环边只在当前 DFS active set 内跳过，visited/load order 仍去重且由 manifest 全量 closure test 锁定。
- 未覆盖与状态：未在真实 CEF 中故意破坏内嵌资源以制造 import failure（自动化 contract 覆盖可重试状态机）；本机未安装 `clang-format`（`Get-Command clang-format` 与常见 VS/LLVM 路径均无结果），因此本轮未运行 format dry-run，MSVC Debug/Release 编译与 `git diff --check` 均通过。其余 MRT-06 Windows 门禁全部闭合，状态转 `DONE`。MRT-08 的合法 CRLF fresh-configure blocker 仍属 KaTeX 后续收口；本任务没有修改或验收 Mermaid、KaTeX 生产能力。
