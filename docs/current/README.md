...
- 当前开发前沿：`CNT-17 DONE` 已在 macOS arm64 产品 CEF 壳接通 Browser-issued 主 frame snapshot request、Renderer DOM adapter、版本化 IPC 与 Browser gateway；`CNT-18a DONE` 已冻结双语言 `content-host-v1` 有界 codec/golden，Rust owner/extract/Markdown host 的 `CNT-18b IN_PROGRESS` 已领取。`PLT-M05b1 READY` 与 `MDV-25 READY` 同样触及 CEF 装配，继续串行。测试与产品启动固定 `use-mock-keychain`，真实 SecureStore/Keychain 验证放到最后，不得作为一期启动障碍。Direct/Relay 可使用 ADB 在线手机的正式接收端取证；Windows x64 同一期回归。Agent/CLI/MCP、Workflow、Hub、Partner、模型与 HarmonyOS 统一为第二期且默认关闭。
