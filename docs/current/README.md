...
- 浏览器内建本地 Markdown 查看器：用户经受控入口打开本地 `.md`，支持源码/渲染预览切换与分栏编辑实时预览、原子保存；`MDV-01..07` 承接，属用户能力，不作为 Agent 能力暴露；契约文档由 `MDV-01` 冻结为 `markdown-viewer.md`。
...
- `browser/engine-api` 的 C++17 跨 CEF/ArkWeb 契约已由 `CEF-01B` 冻结并通过 GCC/MSVC 双编译器与 contract；`CEF-01C` 已建立 Windows/macOS 共用的离线 CEF 构建图，`CEF-01D` 已交付可启动、可关闭且无残留进程的 Windows x64 最小 CEF 壳和受管标题栏品牌图标。当前阶段按用户决策优先把 Windows 浏览器全部基础功能开发并实机跑通：`CEF-02W/CEF-03 DONE` 已交付官方 bootstrap EXE/client DLL 多进程结构、Debug/Release sandbox、共享窗口/标签 owner、Chrome-style 基础导航命令链、品牌图标和无残留进程实机证据；`RNM-01..08 DONE` 已将历史 `get-video` 仓库/package/README/GitHub/本地目录统一迁移为 `crayon-browser` 并完成新路径 CEF configure smoke。`BUX-02 DONE` 已交付平台中立的 Windows UI shell、typed command、focus owner 与 engine event adapter 骨架，并通过 Debug/Release 与 Windows 快捷键/完整退出实机门禁；`BUX-03 DONE` 已交付零默认公网请求的本地 `crayon://newtab`、普通/无痕页面模型、安全 resource handler，以及启动/`Ctrl+T`/`Ctrl+L`/完整退出的 Debug/Release Windows 实机证据；`BUX-04` 已拆分为 `BUX-04A`（核心判定引擎，已就绪）与 `BUX-04B`（provider 配置/隐私集成，仍等待 `PRV-06`）。`CEF-01E VERIFIED` 的 macOS App/Helper Bundle 与 CI 保持冻结，远程双架构/实机证据和正式 sandbox 由 `CEF-02M` 后置，不再阻塞 Windows，也不得用 Windows 证据完成 macOS。`BUX-01` 已冻结 Chrome-inspired 两层信息架构、共享 token、21 个自有功能 glyph 和 8 份规格 golden。`SDK-13 BLOCKED`（需真实接收端 Harness）；`CNT-01..10` 等待浏览器/投屏门禁；`AGT/ACT/WFL/HUB` 分别规划入站访问、语义动作、工作流/挑战和能力生态；`CNT-11..16` 是第二阶段模型能力。
