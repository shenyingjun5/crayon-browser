...
- 当前开发前沿：`CNT-17..18 DONE` 已在 macOS arm64 产品 CEF 壳闭合 Browser-issued request → Renderer DOM adapter → Browser gateway → 真实 Core owner/extract → 确定性 Markdown，并覆盖正常/故障全链。`CNT-19a DONE` 已把显式主 frame 右键用户命令接入真实 snapshot/Core，并复用现有 MDV 预览/编辑会话；完整 CTest 77/77、真实 App 7 进程 loopback-only/零残留。下一切片 `CNT-19b READY` 闭合复制、Save As、覆盖确认和失败反馈，`19c` 做真 UI 验收。`PLT-M05b1 READY` 与 `MDV-25 READY` 同样触及 CEF 装配，继续串行。测试与产品启动固定 `use-mock-keychain`，真实 SecureStore/Keychain 验证放到最后，不得作为一期启动障碍。Direct/Relay 可使用 ADB 在线手机的正式接收端取证；Windows x64 同一期回归。Agent/CLI/MCP、Workflow、Hub、Partner、模型与 HarmonyOS 统一为第二期且默认关闭。
