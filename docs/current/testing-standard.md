...
| L2 Contract/Integration | `tests/contracts`, `tests/integration` | schema、IPC、CEF、Cast-SDK facade、Relay、Markdown | 5 分钟 |
| L3 Security | `tests/security` | SSRF、rebind、重放、路径、secret、旧会话 | 10 分钟 |
| L4 Desktop E2E | `tests/e2e/desktop` | Windows/macOS 浏览、Direct/Relay、外部交接 | 按套件 |
...
当前测试设施不得包含真实模型 provider、FakeModel、FakeAgent、WebRTC、采集器或编码器。自动化不依赖公共网络和第三方影视站。
...
当前权威用例为 [test-cases.md](test-cases.md) 中 95 个唯一 ID。任务必须引用适用 ID，新增/删除用例时同步总 Roadmap 与计划索引。
